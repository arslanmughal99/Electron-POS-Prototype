...
import {StaticItemBill} from '../src/app/interfaces/staticItemsBill-interface';
...
function generatePdf(bill: StaticItemBill, filepath: string) {
    const config: Config = {
    ...
    // const filepath = 'C:\\Users\\arsla\\Desktop\\';
...
function generateHeader(doc, billObj: StaticItemBill, config: Config) {
  ...
  function generateCustomerInformation(doc, bill: StaticItemBill) {
    doc
      .fillColor('#444444')
      .fontSize(20)
      .text('Invoice', 50, 160);
  ...
  function generateInvoiceTable(doc, bill: StaticItemBill, config: Config) {
    let itemsCount = 0;
    ...
    for (i = 0; i < bill.itemsPurchased.length; i++) {
      ...
      generateTableRow(
        ...
        item.itemQuantity,
        item.itemDiscount,
        (item.itemPrice - item.itemDiscount) * item.itemQuantity
      );
  ...
  function calculateTotalPrice (bill: StaticItemBill) {
    const items  = bill.itemsPurchased;
    let total = 0;
    items.forEach(item => {
        total += (item.itemPrice - item.itemDiscount) * item.itemQuantity;
    });
    return total;
  }
...
export function getPdfCommandStatic () {
